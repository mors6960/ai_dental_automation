...
export function Footer() {
  const { t } = useI18n();
  return (
    <footer className="relative overflow-hidden bg-gradient-dark pb-10 pt-20 text-white">
      ...
      <div className="relative mx-auto max-w-7xl px-4 md:px-6">
        <div className="luxury-outline mb-16 flex flex-col items-center justify-between gap-6 rounded-[2rem] glass-dark p-8 md:flex-row md:p-12">
          <div>
            ...
            <p className="mt-2 text-white/70">{t.footer.ctaDescription}</p>
          </div>
          ...
          <div>
            ...
            <div className="mt-5 flex gap-3">
              {[Instagram, Facebook, Twitter].map((I, i) => (
                <a key={i} href="#" className="grid h-9 w-9 place-items-center rounded-full glass-dark transition-colors hover:bg-primary/20">
                  <I className="w-4 h-4" />
                </a>
              ))}
            </div>
        ...
        <div className="mt-12 flex flex-col justify-between gap-3 border-t border-white/10 pt-6 text-xs text-white/45 md:flex-row">
