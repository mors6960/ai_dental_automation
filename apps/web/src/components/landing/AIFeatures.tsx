...
export function AIFeatures() {
  const { t } = useI18n();
  return (
    <section id="ai" className="relative overflow-hidden py-24 text-white">
      ...
      <div className="relative mx-auto max-w-7xl px-4 md:px-6">
        <div className="max-w-2xl">
          ...
          <p className="mt-4 text-lg text-white/72">{t.ai.description}</p>
        </div>
        <div className="mt-12 grid md:grid-cols-2 lg:grid-cols-3 gap-5">
          {t.ai.items.map((f, i) => {
            ...
            <motion.div
              ...
              className="luxury-outline rounded-[1.6rem] glass-dark p-6 transition-all hover:-translate-y-1 hover:border-primary/40 hover:shadow-elegant"
            >
              <div className="grid h-11 w-11 place-items-center rounded-xl bg-gradient-primary shadow-glow">
                <Icon className="w-5 h-5 text-primary-foreground" />
              </div>
              <h3 className="mt-5 text-lg font-semibold text-white">{f.title}</h3>
              <p className="mt-2 text-sm leading-relaxed text-white/68">{f.desc}</p>
            </motion.div>
          )})}
        </div>
      </div>
